    ...
    <a href={link} target="_blank">
      <div className="w-full h-80 rounded-lg bg-[#112240] p-7 flex flex-col justify-between gap-6 hover:-translate-y-2 transition-transform duration-300 group">
        ...
        <div>
          ...
          <p className="text-sm mt-3 h-28 overflow-y-auto scrollbarHide text-justify">{des}</p>
        </div>
        <ul className="text-xs mdl:text-sm text-textDark flex items-center gap-3  flex-wrap">
          {listItem.map((item, i) => (
            <li key={i}>{item}</li>
          ))}
        </ul>
